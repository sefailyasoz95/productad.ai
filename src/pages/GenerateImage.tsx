...
import { Upload, RefreshCw, Image, XCircle, ThumbsUp, ThumbsDown } from "lucide-react";
import DashboardLayout from "@/components/DashboardLayout";
import { useAppDispatch, useAppSelector } from "@/redux/store";
import { clearGeneratedImage } from "@/redux/reducers";
import { generateImage, getAllInfluencers, updateImageLike } from "@/redux/actions";
...
const GenerateImage = () => {
	...
	const [resultLiked, setResultLiked] = useState<boolean | undefined>(undefined);
	...
	useEffect(() => {
		if (resultLiked !== undefined) {
			dispatch(updateImageLike({ liked: resultLiked, image_url: generatedImage }));
		}
	}, [resultLiked]);
						...
						<Card>
							<CardContent className='p-6 space-y-4'>
								<div className='space-y-2'>
									...
									{uploadedImage ? (
										<div className='relative aspect-square rounded-md overflow-hidden border border-border'>
											<img src={uploadedImage} alt='Uploaded product' className='w-full h-full object-contain' />
											<Button
												size='icon'
												variant='destructive'
												...
												<div className='relative'>
													<img
														className={cn(
															...
															influencer.id === influencerId ? "border-primary" : "border-white"
														)}
													...
													<div className='absolute inset-0 bg-black bg-opacity-70 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-center p-2 text-white overflow-y-auto'>
														...
														<small>
															<b>From:</b> <i>{influencer.background.birthplace}</i>
														</small>
													</div>
												</div>
											</div>
								...
								<div className='flex items-center space-x-2 flex-1 justify-between'>
									<Label htmlFor='public-allowed'>Allow other users to see generated image</Label>
									<Switch
										id='public-allowed'
										name='public_allowed'
								...
								<Button
									...
									disabled={loading || !(uploadedImage || sceneDescription || productName)}>
									{loading ? (
										<>
								...
								{generatedImage ? (
									<div className='flex flex-col'>
										<div className='relative rounded-md overflow-hidden border group border-border bg-muted/20'>
											<img src={generatedImage} alt='Generated marketing' className='w-full h-auto object-contain' />
											<div className='absolute bottom-3 w-full px-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-row items-center justify-between'>
												<ThumbsUp
													color='black'
													size={30}
													className={cn("cursor-pointer", resultLiked ? "fill-primary" : "fill-white")}
													onClick={() => setResultLiked(true)}
												/>
												<div className='bg-white/20 backdrop-blur-sm px-2 py-1 rounded-lg'>
													<span className='font-medium text-white'>Good result ?</span>
												</div>
												...
												<ThumbsDown
													...
													color='black'
													size={30}
													className={cn("cursor-pointer", resultLiked === false ? "fill-primary" : "fill-white")}
												/>
											</div>
										</div>
