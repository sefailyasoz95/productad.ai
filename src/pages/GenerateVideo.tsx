...
import { Upload, RefreshCw, Download, Video, XCircle, Play, Pause, ArrowRight, Image } from "lucide-react";
import { useToast } from "@/components/ui/use-toast";
import DashboardLayout from "@/components/DashboardLayout";
import { useAppDispatch, useAppSelector } from "@/redux/store";
import { generateVideo, getRecentGenerations } from "@/redux/actions";
...
const GenerateVideo = () => {
	...
	useEffect(() => {
		dispatch(getRecentGenerations(undefined));
	}, []);
	...
	console.log("selectedInfluencerImage: ", selectedInfluencerImage);
	...
	const handleGenerate = () => {
		...
		dispatch(
			generateVideo({
				...
				description,
				influencerId,
				productImage: selectedInfluencerImage,
				productName,
				publicAllowed: true,
				sceneDescription,
			})
							...
							<CardContent className='p-6 space-y-4'>
								<Tabs defaultValue='upload' className='w-full'>
									...
									<TabsContent value='recent' className='mt-4 space-y-4'>
										<div className='space-y-2'>
											<Label htmlFor='style'>Select from previously generated images</Label>
											<div className='flex flex-row gap-x-2 overflow-scroll flex-nowrap max-w-fit'>
												{recentGenerations.map((generation) => (
													<div
														onClick={() => {
															if (generation.images.influencer_id === influencerId) {
																setSelectedInfluencerImage("");
																setInfluencerId(null);
															} else {
																setSelectedInfluencerImage(generation.images.image_url);

																setInfluencerId(generation.images.influencer_id);
															}
														}}
														className={cn("flex flex-col min-w-40 rounded-lg cursor-pointer relative group")}>
														<div className='relative'>
															<img
																className={cn(
																	"w-full object-cover border-2 rounded-lg",
																	generation.images.influencer_id === influencerId ? "border-primary" : "border-white"
																)}
																src={generation.images.image_url}
																alt={generation.images.product_name}
															/>

															{/* Hover overlay - absolute positioned on top of the image */}
															<div className='absolute text-sm inset-0 bg-black bg-opacity-70 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-center p-2 text-white overflow-y-auto'>
																<div className='flex items-center flex-col'>
																	<b>Product Name</b> <i>{generation.images.product_name}</i>
																</div>
															</div>
														</div>
													</div>
