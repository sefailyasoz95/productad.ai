...
import { ArrowRight, ImageIcon, Video, Mic, ChevronLeft, ChevronRight } from "lucide-react";
...
const Hero = () => {
	...
	const navigateVideo = (direction) => {
		if (direction === "next") {
			...
			setCurrentVideoIndex((prevIndex) => (prevIndex === 0 ? demoVideos.length - 1 : prevIndex - 1));
		}
	};
				...
				{demoVideos.map((_, index) => (
					<div
						...
						className={`w-2 h-2 rounded-full cursor-pointer transition-all ${
							index === currentVideoIndex ? "w-6 bg-brand-purple" : "bg-white/60"
						}`}
			...
			<div className='relative z-10 flex flex-col items-center justify-center lg:h-full mt-24 lg:mt-0 max-w-6xl mx-auto px-6 text-white'>
					...
					<div className='flex justify-center opacity-0 animate-fade-in' style={{ animationDelay: "0.5s" }}>
						<Dialog>
							...
							<DialogContent className='sm:max-w-md'>
								<DialogHeader>
									<DialogTitle className='text-xl text-center'>Sign in to ProductAd.ai</DialogTitle>
								</DialogHeader>
				...
				<div className='grid grid-cols-1 md:grid-cols-3 gap-6 mt-64 lg:mt-16'>
					<div className='backdrop-blur-md bg-white/10 rounded-lg p-6 text-center flex flex-col items-center transform transition-all hover:scale-105'>
						...
						<p className='text-white/80'>Generate stunning product showcases with our advanced AI</p>
					</div>
					...
					<div className='backdrop-blur-md bg-white/10 rounded-lg p-6 text-center flex flex-col items-center transform transition-all hover:scale-105'>
						...
						<p className='text-white/80'>Create engaging videos with or without virtual influencers</p>
					</div>
					...
					<div className='backdrop-blur-md bg-white/10 rounded-lg p-6 text-center flex flex-col items-center transform transition-all hover:scale-105'>
						...
						<p className='text-white/80'>Add lifelike voiceovers or let our influencers talk for you</p>
					</div>
				</div>
			</div>
		</section>
