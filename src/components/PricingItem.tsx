...
export default function PricingItem({ product, onItemSelect, isCurrent }: Props) {
				...
				<CardHeader className=' pb-0'>
					<div className='flex justify-between items-center'>
						<div className='flex items-center gap-2'>
							...
							<CardTitle className='text-2xl font-bold text-violet-800'>{product.name}</CardTitle>
						</div>
						<Badge className='bg-violet-100 text-violet-800 hover:bg-violet-200 capitalize'>
							{isCurrent ? "Current Plan" : product.prices[1].recurring.interval + "ly"}
						</Badge>
					</div>
					<CardDescription className='text-gray-600 mt-2 text-xs font-medium italic'>
						{product.description}
					</CardDescription>
				</CardHeader>
              ...
              >
					...
					<div className='text-center mb-8'>
						<div className='inline-flex items-baseline'>
							<span className='text-5xl font-extrabold text-violet-800'>
								{formatCurrency(product.prices[1].unit_amount / 100, product.prices[1].currency)}
							</span>
							<span className='ml-1 text-gray-500'>/ {product.prices[1].recurring.interval}</span>
						</div>
					</div>

					<div className='space-y-4 mb-6'>
						{product.marketing_features.map((feature, index) => (
							<div key={index} className='flex items-center bg-violet-50 p-3 rounded-md'>
				...
				<CardFooter className='px-6 py-4'>
					<Button
						...
						<span>
							{isCurrent === undefined ? "Start Creating" : isCurrent ? "Current plan" : "Update to this plan"}
						</span>
					</Button>
				</CardFooter>
			</Card>
